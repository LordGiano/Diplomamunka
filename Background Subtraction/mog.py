...
backSub = cv.bgsegm.createBackgroundSubtractorMOG(history=args.history, nmixtures=args.nmixtures,
                                                  backgroundRatio=args.backgroundRatio, noiseSigma=args.noiseSigma)
...
while True:
    ...
    if keyboard == ord(' '):  # Space megnyomása
        ...
        if last_filtered_fgMask is not None:
            save_screenshot("MOG", last_frame, last_filtered_fgMask, frame_index)
