...
backSub = cv.bgsegm.createBackgroundSubtractorLSBP(LSBPRadius=args.LSBPRadius, Tlower=args.Tlower, Tupper=args.Tupper, minCount=args.minCount)
...
while True:
    ...
    if keyboard == ord(' '):  # Space megnyomása
        ...
        if last_filtered_fgMask is not None:
            save_screenshot("LSBP", last_frame, last_filtered_fgMask, frame_index)
