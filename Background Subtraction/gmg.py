...
backSub = cv.bgsegm.createBackgroundSubtractorGMG(initializationFrames=args.initializationFrames,
                                                  decisionThreshold=args.decisionThreshold)
...
while True:
    ...
    if keyboard == ord(' '):  # Space megnyomása
        ...
        if last_filtered_fgMask is not None:
            save_screenshot("GMG", last_frame, last_filtered_fgMask, frame_index)
